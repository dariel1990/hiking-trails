                    ...
                    <div class="space-y-2" x-show="locationType === 'trail'" x-cloak>
                        ...
                        <select name="trail_network_id"
                            ...
                            @foreach($trailNetworks as $network)
                                <option value="{{ $network->id }}" {{ old('trail_network_id', request('network')) == $network->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2 grid grid-cols-[200px_1fr_1fr] gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="number" name="fishing_distance_from_town" value="{{ old('fishing_distance_from_town') }}"
            ...
            <div class="p-6 space-y-6">
                ...
                <div id="photo-upload-zone" class="border-2 border-dashed border-input rounded-lg p-8 text-center hover:border-primary transition-colors cursor-pointer">
                    ...
                    <div id="upload-prompt">
                        ...
                        <p class="text-xs text-muted-foreground mt-1">PNG, JPG, GIF up to 50MB (max 10 photos)</p>
                    </div>
                </div>
...
<script>
    ...
    class TrailBuilder {
        ...
        loadExistingTrails() {
            ...
            existingTrails.forEach(trail => {
                if (trail.coordinates && trail.coordinates.length > 1) {
                    try {
                        // coordinates are [lat,lng]; GeoJSON needs [lng,lat]
                        features.push({
                            type: 'Feature',
        ...
        selectSearchResult(index) {
            ...
            // Fly to location
            this.map.flyTo({ center: [lon, lat], zoom: 15, duration: 1500 });
        ...
        addSearchMarker(lat, lon, name) {
            ...
            const el = document.createElement('div');
            el.style.cssText = 'width:20px;height:20px;background:#2563EB;border-radius:50%;border:2px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.4);';

            const popup = new mapboxgl.Popup({ offset: 14 }).setHTML(`<b>Search Result</b><br>${name}`);
            this.searchMarker = new mapboxgl.Marker({ element: el })
                .setLngLat([lon, lat])
                .setPopup(popup)
                .addTo(this.map);
            this.searchMarker.togglePopup();

            setTimeout(() => { this.removeSearchMarker(); }, 10000);
        }
