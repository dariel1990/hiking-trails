...
<div class="space-y-6">
    ...
    {{-- Existing Media --}}
    <div id="existing-media-section" class="rounded-lg border bg-card text-card-foreground shadow-sm @if($business->media->isEmpty()) hidden @endif">
        <div class="flex flex-col space-y-1.5 p-6 border-b">
            <h3 class="text-lg font-semibold leading-none tracking-tight">Existing Media</h3>
            <p id="existing-media-count" class="text-sm text-muted-foreground">{{ $business->media->count() }} item{{ $business->media->count() !== 1 ? 's' : '' }}</p>
        </div>
        <div class="p-6">
            <div id="existing-media-grid" class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                @foreach($business->media as $media)
                    <div class="group relative overflow-hidden rounded-lg border bg-card shadow-sm" data-media-id="{{ $media->id }}">
                        ...
                        <div class="p-2 space-y-1.5">
                            ...
                            @endif
                            <div class="flex items-center gap-1" data-media-actions>
                                @if(!$media->is_primary)
                                    <button type="button" title="Set as primary"
                                        onclick="setPrimaryBusinessMedia({{ $business->id }}, {{ $media->id }}, this)"
                                        class="flex-1 w-full inline-flex h-6 items-center justify-center rounded border border-input bg-background text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                                        Primary
                                    </button>
                                @endif
                                <button type="button" title="Delete"
                                    onclick="deleteBusinessMedia({{ $business->id }}, {{ $media->id }}, this)"
