...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div class="space-y-1">
            <h2 class="text-2xl font-semibold tracking-tight">Tours</h2>
            <p class="text-sm text-muted-foreground">Manage themed tour packages that appear on the site</p>
        </div>
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
            ...
            <div class="text-2xl font-bold">{{ $tours->count() }}</div>
        </div>
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
            <div class="flex items-center justify-between pb-2">
            ...
            <div class="text-2xl font-bold text-green-600">{{ $tours->where('is_active', true)->count() }}</div>
        </div>
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
            <div class="flex items-center justify-between pb-2">
            ...
            <div class="text-2xl font-bold text-yellow-500">{{ $tours->where('is_featured', true)->count() }}</div>
        </div>
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
            <div class="flex items-center justify-between pb-2">
            ...
            <div class="text-2xl font-bold text-blue-500">{{ $tours->pluck('tour_type')->unique()->count() }}</div>
        </div>
    </div>
    ...
    <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
        @if($tours->isEmpty())
            <div class="p-12 text-center">
                <div class="flex flex-col items-center justify-center space-y-4">
                    ...
                    <div class="space-y-2">
                        <h3 class="text-lg font-semibold">No tours yet</h3>
                        <p class="text-sm text-muted-foreground">Create your first themed tour experience.</p>
                    </div>
                    <a href="{{ route('admin.tours.create') }}"
        ...
        @else
            <form id="bulk-form" method="POST" action="{{ route('admin.tours.bulk-action') }}" class="hidden">
                @csrf
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b bg-muted/50">
                            ...
                            <th class="h-12 px-6 text-left font-medium text-muted-foreground">Tour</th>
                            <th class="h-12 px-4 text-left font-medium text-muted-foreground">Type</th>
                            <th class="h-12 px-4 text-left font-medium text-muted-foreground">Stops</th>
                    ...
                    <tbody class="divide-y">
                        @foreach($tours as $tour)
                            <tr class="hover:bg-muted/30 transition-colors">
                                <td class="px-4 py-4">
                                    <input type="checkbox" data-id="{{ $tour->id }}"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @else
                                            <div class="flex h-10 w-10 items-center justify-center rounded bg-muted text-lg flex-shrink-0">🗺️</div>
                                        @endif
                                        <div>
                                            <div class="font-medium">{{ $tour->title }}</div>
                                            @if($tour->tagline)
                                                <div class="text-xs text-muted-foreground truncate max-w-[200px]">{{ $tour->tagline }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex flex-col gap-2">
                                        <button type="button"
                                            ...
                                            <span class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors {{ $tour->is_active ? 'bg-green-500' : 'bg-gray-300' }}">
                                                <span class="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform {{ $tour->is_active ? 'translate-x-4' : 'translate-x-0.5' }}"></span>
                                            </span>
                                            <span class="toggle-label text-xs font-semibold {{ $tour->is_active ? 'text-green-700' : 'text-gray-500' }}">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.tours.destroy', $tour) }}"
                                            onsubmit="return confirmDelete(event, '{{ $tour->title }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
...
function confirmDelete(event, name) {
    ...
    document.getElementById('confirm-modal-message').textContent = 'This will permanently delete this tour and all its stops. This cannot be undone.';
...
function submitBulkAction(action) {
    ...
    const labels = {
        delete: { title: 'Delete selected tours?', msg: 'This will permanently delete ' + selected.length + ' tour(s). This cannot be undone.' },
        activate: { title: 'Mark ' + selected.length + ' tour(s) as Active?', msg: 'They will become visible on the site.' },
        deactivate: { title: 'Mark ' + selected.length + ' tour(s) as Inactive?', msg: 'They will be hidden from the site.' },
    };
