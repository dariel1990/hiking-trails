...
@section('title', 'Create Trail Network')
@section('page-title', 'Create Trail Network')

@section('content')
<link href="https://api.mapbox.com/mapbox-gl-js/v3.10.0/mapbox-gl.css" rel="stylesheet">

<div x-data="iconPicker('{{ old('icon', '') }}')" class="px-4 lg:px-8 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 mb-6 border-b border-gray-200">
        <div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 leading-tight">Create Trail Network</h1>
                    <p class="text-sm text-gray-500">Add a new network of trails to the map</p>
                </div>
            </div>
        </div>
    ...
    <form id="trail-network-form" action="{{ route('admin.trail-networks.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="px-6 py-5 space-y-5">
                        <div>
                            ...
                            <input type="text" name="network_name" id="network_name"
                                   value="{{ old('network_name') }}" required
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" required
                                    ...
                                    <option value="nordic_skiing" {{ old('type') === 'nordic_skiing' ? 'selected' : '' }}>Nordic Skiing</option>
                                    <option value="downhill_skiing" {{ old('type') === 'downhill_skiing' ? 'selected' : '' }}>Downhill Skiing</option>
                                    <option value="hiking" {{ old('type') === 'hiking' ? 'selected' : '' }}>Hiking</option>
                                    <option value="mountain_biking" {{ old('type') === 'mountain_biking' ? 'selected' : '' }}>Mountain Biking</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="px-6 py-5 space-y-5">
                        <div>
                            ...
                            <textarea name="description" id="description" rows="5"
                                      ...
                                      class="block w-full rounded-lg border-gray-300 shadow-sm px-4 py-2.5 focus:border-green-500 focus:ring-green-500 @error('description') border-red-300 @enderror">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="xl:col-span-5 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div class="px-6 py-5 space-y-5">
                        ...
                        @include('admin.trail-networks._video-field', ['trailNetwork' => null])
                    </div>
                </div>

                @include('admin.trail-networks._sponsors-section', ['sponsors' => []])
            </div>
        </div>
    </form>
</div>
