            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="facility_type" 
                        ...
                        <option value="parking" {{ old('facility_type', $facility->facility_type) === 'parking' ? 'selected' : '' }}>🅿️ Parking</option>
                        <option value="toilets" {{ old('facility_type', $facility->facility_type) === 'toilets' ? 'selected' : '' }}>🚻 Toilets</option>
                        <option value="emergency_kit" {{ old('facility_type', $facility->facility_type) === 'emergency_kit' ? 'selected' : '' }}>🏥 Emergency Kit</option>
                        <option value="lodge" {{ old('facility_type', $facility->facility_type) === 'lodge' ? 'selected' : '' }}>🏠 Lodge</option>
                        <option value="viewpoint" {{ old('facility_type', $facility->facility_type) === 'viewpoint' ? 'selected' : '' }}>👁️ Viewpoint</option>
                        <option value="info" {{ old('facility_type', $facility->facility_type) === 'info' ? 'selected' : '' }}>ℹ️ Information</option>
                        <option value="picnic" {{ old('facility_type', $facility->facility_type) === 'picnic' ? 'selected' : '' }}>🍽️ Picnic Area</option>
                        <option value="water" {{ old('facility_type', $facility->facility_type) === 'water' ? 'selected' : '' }}>💧 Water</option>
                        <option value="shelter" {{ old('facility_type', $facility->facility_type) === 'shelter' ? 'selected' : '' }}>⛺ Shelter</option>
                    </select>
                    @error('facility_type')
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-sm text-gray-600 mb-3">Click on the map or drag the marker to update the facility location</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" 
                              ...
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 @error('description') border-red-300 @enderror">{{ old('description', $facility->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Current icon: <span id="icon-preview">{{ $facility->icon }}</span></p>
                    @error('icon')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Click on map to update facility location
