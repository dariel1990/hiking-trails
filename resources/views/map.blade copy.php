...
/* Selected state for trail/business/fishing/highlight markers — styled in place
   so the marker never shifts off its lat/lng. */
...
/* Network markers always on top of other markers */
.network-marker-el {
    z-index: 10;
    position: relative;
    transition: transform 0.15s ease, box-shadow 0.15s ease;
...
<script>
    ...
    class EnhancedTrailMap {
        ...
        _createNetworkMarkerEl(emoji) {
            ...
            el.dataset.emoji = emoji;
            el.style.cssText = 'background-color:#166534;width:38px;height:38px;border-radius:8px;border:2px solid #ffffff;box-shadow:0 3px 10px rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;font-size:17px;cursor:pointer;line-height:1;z-index:10;position:relative;';
            el.textContent = emoji;
            return el;
        }
