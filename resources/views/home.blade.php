...
<section class="relative min-h-screen flex items-center justify-center hero-gradient overflow-hidden">
    ...
    <div class="relative z-30 text-center text-white max-w-6xl mx-auto px-4 flex flex-col justify-center min-h-screen py-20">
        ...
        <div class="slide-in-up mb-10" style="animation-delay: 0.2s;">
        ...
        {{-- Home's panel is a launcher, not a filter of anything on this page:
             it posts straight to /trails. No result count is passed, so the
             component omits the "N found" line. This also retires a crash -
             the old panel referenced $trails, which home() never provided, so
             any visit to /?search=... returned a 500. --}}
        <x-filter-panel
            :action="route('trails.index')"
            search-label="Search adventures"
            search-placeholder="Trail name, location…"
            submit-label="Find trails"
            :filters="[
