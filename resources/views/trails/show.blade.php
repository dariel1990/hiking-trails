...
<!-- GPX download toast — slides up from the bottom with the same slow-mo timing as the modals below -->
<style>
    @keyframes xsGpxToastIn { from { transform: translate(-50%, 100%); opacity: 0; } to { transform: translate(-50%, 0); opacity: 1; } }
    @keyframes xsGpxToastOut { from { transform: translate(-50%, 0); opacity: 1; } to { transform: translate(-50%, 100%); opacity: 0; } }

    .xs-gpx-toast {
        position: fixed; left: 50%; bottom: 24px; z-index: 60;
        display: flex; align-items: center; gap: 8px;
        background: #1f2937; color: #fff; font-size: 14px; font-weight: 600;
        padding: 12px 20px; border-radius: 999px; box-shadow: 0 10px 30px rgba(0,0,0,.3);
        pointer-events: none; transform: translate(-50%, 100%); opacity: 0;
    }
    .xs-gpx-toast.is-in { animation: xsGpxToastIn 0.8s cubic-bezier(.22,1,.36,1) forwards; }
    .xs-gpx-toast.is-out { animation: xsGpxToastOut 0.8s cubic-bezier(.22,1,.36,1) forwards; }
</style>

<div id="gpx-toast" class="xs-gpx-toast">
...
<!-- Share Modal — styled to match the app-download / Pro upgrade modals -->
<style>
    ...
    .xs-share-modal { position: fixed; inset: 0; z-index: 9500; display: none; align-items: flex-end; justify-content: center; pointer-events: none; }
...
<div id="share-modal" class="xs-share-modal" aria-hidden="true">
    <div class="xs-share-backdrop" data-xs-share-close></div>
    <div class="xs-share-dialog" role="dialog" aria-modal="true" aria-labelledby="xs-share-title">
        <div class="xs-share-head">
            <button type="button" id="close-share-modal" class="xs-share-x" aria-label="Close">&times;</button>
            <p class="xs-share-kicker">Share This Trail</p>
            <h2 id="xs-share-title" class="xs-share-title">{{ $trail->name }}</h2>
            <p class="xs-share-sub">{{ $trail->location }}</p>
        </div>
        ...
        <div class="xs-share-body">
            <div class="xs-share-link-row">
                ...
                <button id="copy-link-btn" class="xs-share-copy-btn">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="xs-share-actions">
            <button onclick="shareToFacebook()" class="xs-share-cta" style="background:#1877F2;">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
                Facebook
            </button>
            <button onclick="shareToTwitter()" class="xs-share-cta" style="background:#000;">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
                X (Twitter)
            </button>
            <button onclick="shareToWhatsApp()" class="xs-share-cta" style="background:#25D366;">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <button onclick="shareViaEmail()" class="xs-share-cta" style="background:#4b5563;">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const gpxToast = document.getElementById('gpx-toast');
    let gpxToastHideTimeout = null;

    function showGpxToast() {
        if (gpxToastHideTimeout) { clearTimeout(gpxToastHideTimeout); gpxToastHideTimeout = null; }
        gpxToast.classList.remove('is-out');
        gpxToast.classList.add('is-in');
        gpxToastHideTimeout = setTimeout(() => {
            gpxToast.classList.remove('is-in');
            gpxToast.classList.add('is-out');
            gpxToastHideTimeout = null;
        }, 2600);
    }
    ...
    let shareModalHideTimeout = null;
    ...
    function openShareModal() {
        ...
        if (shareModalHideTimeout) { clearTimeout(shareModalHideTimeout); shareModalHideTimeout = null; }
        // is-open's keyframe @animation plays in full from its "from" state even when
        // added in the same tick as is-visible, so no reflow/rAF trick is needed here
        // (unlike a plain CSS transition, which needs the "before" state already painted).
        shareModal.classList.add('is-visible', 'is-open');
        shareModal.setAttribute('aria-hidden', 'false');
        document.body.style.overflow = 'hidden';
    }

    function closeShareModal() {
        if (shareModalHideTimeout) { clearTimeout(shareModalHideTimeout); }
        shareModal.classList.remove('is-open');
        shareModal.setAttribute('aria-hidden', 'true');
        document.body.style.overflow = '';
        shareModalHideTimeout = setTimeout(() => {
            shareModal.classList.remove('is-visible');
            shareModalHideTimeout = null;
        }, 1100);
    }
    ...
    shareModal?.addEventListener('click', function(e) {
        if (e.target.closest('[data-xs-share-close]')) { closeShareModal(); }
    });
