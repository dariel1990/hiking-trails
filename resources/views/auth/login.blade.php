...
<div class="auth-stage flex items-center justify-center px-4 py-10 font-sans">
    ...
    <div class="auth-card relative w-full max-w-md bg-white rounded-3xl overflow-hidden shadow-[0_30px_80px_-20px_rgba(10,30,28,0.65)]">
        ...
        <div class="auth-head px-8 pt-9 pb-8 text-center">
            ...
            <p class="relative mt-5 text-[11px] font-semibold uppercase tracking-[0.22em] text-accent-500">Welcome back</p>
            <h1 class="relative mt-2 text-2xl font-bold text-white tracking-tight" style="text-wrap:balance">Sign in to your account</h1>
            <p class="relative mt-2 text-sm text-white/65 leading-relaxed">Pick up where you left off — your trails, lakes, and Pro features.</p>
        </div>
        ...
        <div class="px-7 sm:px-8 pt-7 pb-8">
            ...
            <form class="space-y-4" method="POST" action="{{ route('login.post') }}" x-data="{ showPassword: false }">
                @csrf
                ...
                <div class="flex items-center justify-between gap-3">
                    <label class="inline-flex items-center gap-2 text-sm text-gray-600 cursor-pointer select-none">
                        <input type="checkbox" name="remember" value="1" class="h-4 w-4 rounded border-gray-300 text-forest-600 focus:ring-forest-600/30">
                        Keep me signed in
                    </label>
                    <a href="{{ route('password.request') }}" class="text-sm font-medium text-forest-700 hover:text-accent-600 transition">Forgot password?</a>
                </div>
